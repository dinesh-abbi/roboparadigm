...
export function Model(props) {
  ...
  const setRef = (el, name, order, zone) => {
    if (el && !meshRefs.current[name]) {
      ...
      // Clone material so we can fade its opacity individually per part
      if (el.material) {
      ...
      // Add a glowing wireframe duplicate child of this mesh
      if (el.geometry && !el.userData.wireframeClone) {
        ...
        const clone = new THREE.Mesh(el.geometry, wireframeMat)
        el.add(clone)
        el.userData.wireframeClone = clone
      }
    } else if (el) {
      meshRefs.current[name] = el
    }
