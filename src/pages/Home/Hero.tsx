...
import {
  ...
  SUBSYSTEMS,
  type SubsystemKey,
} from "@/components/robot/PartSimulator";
...
export function ResponsiveArm({ children }: { children: React.ReactNode }) {
...
export function ScrollCameraRig({ progress, controlsRef, simulatorActive }: ScrollCameraRigProps) {
  const { camera } = useThree();
  ...
  useFrame(() => {
    // Once simulator is active (user selected a subsystem), hand control to PartCameraRig
...
function CornerBracket({ position, delay }: { position: "tl" | "tr" | "bl" | "br"; delay: number }) {
...
export function Hero() {
  ...
  // Part simulator state
  const [selectedSubsystem, setSelectedSubsystem] = useState<SubsystemKey>("ALL");
  ...
  useEffect(() => {
    ...
    const unsub = assemblyProgress.on("change", (v) => {
      rawProgress.set(v);
      if (v >= 0.99 && !assemblyDone) setAssemblyDone(true);
    });
    return () => { controls.stop(); unsub(); };
  }, []);

  const handleSelectSubsystem = (key: SubsystemKey) => {
    setSelectedSubsystem(key);
    setSelectedMesh(null);
    setSimulatorActive(true);
    // Re-enable orbit after fly-in
    if (controlsRef.current) {
      controlsRef.current.autoRotate = key === "ALL";
    }
  };

  const handleReset = () => {
    handleSelectSubsystem("ALL");
  ...
  // Compute which meshes belong to highlighted subsystem
  const highlightSubsystem = selectedSubsystem !== "ALL" ? selectedSubsystem : null;
    ...
    <section className="relative w-full h-[100vh] md:h-[100svh] overflow-hidden bg-background">
      ...
      >
        ...
        >
          ...
          <Suspense fallback={null}>
            <ResponsiveArm>
              <RoboarmModel
                ...
                clickedMesh={selectedMesh}
                highlightedSubsystem={highlightSubsystem}
                debugFullModel={editMode}
              />
          ...
          <CameraLogger />
          <ScrollCameraRig progress={s} controlsRef={controlsRef} simulatorActive={simulatorActive} />
          <PartCameraRig
            selectedSubsystem={simulatorActive ? selectedSubsystem : null}
            controlsRef={controlsRef}
            enabled={assemblyDone && simulatorActive}
          />
          <OrbitControls
            ref={controlsRef}
            enableZoom={false}
      ...
      {/* ── Layer 8: Part Simulator Panel (desktop) ─────────────────────────────── */}
      {!isMobile && (
        <PartSimulatorPanel
          visible={assemblyDone}
